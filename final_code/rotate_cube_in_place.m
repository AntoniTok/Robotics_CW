...
%% 2. GRIPPER SETTINGS 
% Define the offset in radians from 180 degrees 
...
try
    for i = 1:size(cubes_start, 1)
        ...
        if i == 2 || i == 3
            ...
            % Execute pre-rotation waypoints — FIX: gripper action outside valid_t
            for wp_idx = 1:size(pre_waypoints, 1)
                ...
                for t = 1:num_steps
                    ...
                    % FIX: Gripper actions fire at end of waypoint regardless of IK validity
                    if t == num_steps
                        if action == 1      % PICK
                            attached_cube_idx = i;
        ...
        if under_bridge
          ...
          if under_bridge
            ...
            waypoints = [
                ...
                entry_x,  entry_y,  z_corridor+0.01,   pick_pitch, 0;
                grip_x,   grip_y,   z_corridor+0.01,   pick_pitch, 0;
                grip_x,   grip_y,   cz_pick,           pick_pitch, 0;
                grip_x,   grip_y,   cz_pick,           pick_pitch, 1;
                grip_x,   grip_y,   cz_pick+0.02,           pick_pitch, 1;
                exit_x,   exit_y,   z_corridor+0.05,   pick_pitch, 2;
                exit_x,   exit_y,   SAFE_Z_ABOVE,      pick_pitch, 2;
                hx,       hy,       cz_place+hover_z,  pick_pitch, 2;

                % FIX: Use best_pitch (validated) instead of pick_pitch-(pi/2) (unvalidated)
                hx+0.003, hy-0.007, cz_place+0.01,     best_pitch-(pi/10), 2;
                hx+0.003, hy-0.007, cz_place+0.01,     best_pitch-(pi/10), 3;
                hx+0.003, hy-0.007, cz_place+0.01,     best_pitch-(pi/10), 3;
                hx+0.003, hy-0.007, cz_place+0.01,     best_pitch, 3;
                hx_place, hy_place,  cz_place,          best_pitch, 3;

                % retract with gripper already open
                hx_place, hy_place, cz_place+hover_z,  best_pitch, 3;
                hx_place, hy_place, travel_z,          travel_pitch, 0;
                home_x,   home_y,   travel_z,          travel_pitch, 0;
            ];
        else
            ...
            waypoints = [
                ...
                cx, cy, cz_pick+0.01,            best_pitch, 1;   % Pick
        ...
        % FIX: Main waypoint loop — gripper action outside valid_t guard
