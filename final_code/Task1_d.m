...
for i = 1:num_waypoints
    fprintf('Waypoint %d/%d: (%.3f, %.3f, %.3f) pitch=%.1f°\n', i, num_waypoints, all_waypoints(i,1), all_waypoints(i,2), all_waypoints(i,3), rad2deg(pitch_angles(i)));
    
    try
        [theta1, theta2, theta3, theta4, theta5] = inverse_kinematics_continuous(all_waypoints(i,1), all_waypoints(i,2), all_waypoints(i,3), pitch_angles(i), theta_prev, joint_limits);
...
for waypoint_idx = 1:(num_waypoints-1)
    ...
    for i = 1:length(t_vec)
        ...
        text(0.22, 0.22, 0.35, sprintf('Drawing: %s', square_name), 'FontSize', 11, 'FontWeight', 'bold', 'BackgroundColor', 'white');
