...
% FIX 1: Restored SWING_VEL from 1023 back to 900 to avoid erratic motor
%         behaviour at the true hardware limit.
% FIX 2: Restored SWING_ACCEL from 120 back to 80 to avoid motor faults
%         or clipped acceleration profiles.
...
% Read actual current position and pitch from forward kinematics
...
% FIX 4: Read actual position and pitch from forward_kinematics after the
%         first approach loop rather than hardcoding current_pos_val = HOME_HIGH.
%         If any IK steps were skipped in the loop above, the arm may not
%         have reached HOME_HIGH exactly, so hardcoding it would cause a
%         discontinuous jump at the start of the second interpolation.
current_pos_val   = forward_kinematics(current_q, d1,a2,a3,L_tip_total,delta);
current_pitch_val = current_q(2)+delta + current_q(3) + current_q(4);
