...
for g_idx = 1:size(gates, 1)
    if ~ischar(gates{g_idx, 3}) || ~ismember(lower(gates{g_idx, 3}), {'x', 'y'})
        error('Invalid gate orientation at row %d. Use x or y.', g_idx);
    end
end
...
phys_angles = sim_to_phys_angles(current_q, current_gripper, delta, offset_classmate, false, motor11_offset);
...
try
    ...
    for wp_idx = 1:size(pick_wps, 1)
        wp_row    = pick_wps(wp_idx, :);
        goal_x    = wp_row(1); goal_y = wp_row(2); goal_z = wp_row(3);
        goal_pitch = wp_row(4); grip_cmd = wp_row(5);
        ...
        traj_pitch = linspace(curr_pitch, goal_pitch, num_steps);
        ...
        for t = 1:num_steps
            ...
            if valid_t
                ...
                if t == num_steps && grip_cmd == 1
                    current_gripper = GRIPPER_CLOSE;
                    phys_angles = sim_to_phys_angles(current_q, current_gripper, delta, offset_classmate, true, motor11_offset);
                    send_to_robot(port_num, PROTOCOL_VERSION, IDs, phys_angles);
                    pause(0.5);
                end

                phys_angles = sim_to_phys_angles(current_q, current_gripper, delta, offset_classmate, current_gripper == GRIPPER_CLOSE, motor11_offset);
                send_to_robot(port_num, PROTOCOL_VERSION, IDs, phys_angles);
...
%% 7. Gate Traversal
try
    for gate_idx = 1:size(gates, 1)

        g_x = gates{gate_idx, 1};
        g_y = gates{gate_idx, 2};
        g_ori = gates{gate_idx, 3};
        g_z_pass = gates{gate_idx, 4};
        ...
        switch g_ori
            ...
            otherwise
                error('Unexpected gate orientation at gate %d', gate_idx);
        end

        fprintf('Passing Gate %d at Grid(%d,%d) at height %.3fm\n', gate_idx, g_x, g_y, g_z_pass);
        ...
        if gate_idx == 1
            ...
            % The path is split on purpose: keep the old Y first, then fix Z, then re-align Y.
            % This is less fancy than one-shot interpolation, but it prevents side-swipe near the posts.
            gate_wps = [
        ...
        for wp_idx = 1:size(gate_wps, 1)
            wp_row     = gate_wps(wp_idx, :);
            goal_x     = wp_row(1); goal_y = wp_row(2); goal_z = wp_row(3);
            goal_pitch = wp_row(4);
            ...
            for t = 1:num_steps
                ...
                if valid_t
                    ...
                    warning('Trajectory point unreachable for Gate %d!', gate_idx);
                end
            end
        end
...
try
    ...
    pick_pitch = -pi/4; % Hanging straight down
    ...
    for wp_idx = 1:size(drop_wps, 1)
        wp_row    = drop_wps(wp_idx, :);
        goal_x    = wp_row(1); goal_y = wp_row(2); goal_z = wp_row(3);
        goal_pitch = wp_row(4); grip_cmd = wp_row(5);
...
function phys_angles = sim_to_phys_angles(sim_q, gripper_q, delta, offset_classmate, is_placing, motor11_offset)
if numel(sim_q) < 4 || ~isfinite(gripper_q) || ~isfinite(motor11_offset)
    error('sim_to_phys_angles received invalid inputs.');
end
if is_placing
    if sim_q(1) > 0
        q1 = sim_q(1) - motor11_offset;
    else
        q1 = sim_q(1);
    end
else
...
if numel(IDs) < 5 || numel(phys_angles) < 5
    error('send_to_robot expects 5 IDs and 5 target angles.');
end
...
isValid  = true;
if any(~isfinite([x, y, z, phi, d1, a2, a3, L4, delta])) || a2 <= 0 || a3 <= 0 || L4 <= 0
    isValid = false;
