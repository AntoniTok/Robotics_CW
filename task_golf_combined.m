% task_golf_combined.m
% 1. Pick one ball from holder -> place on golf tee -> pick golf club
% 2. Executes a parabolic golf-swing motion
...
BAUDRATE   = 1000000;
DEVICENAME = 'COM7';                 % Matching task_golf.m
...
SWING_VEL            = 1023;  % Max velocity for standard Dynamixels
...
try
    ...
    waypoints_ball = [
        bx, by, ball_pick_z + hover_z, best_pitch, 0, 0;   % approach ball, gripper open
        bx, by, ball_pick_z,           best_pitch, 1, 0;   % close on ball
        bx, by, ball_pick_z + hover_z, best_pitch, 2, 0;   % lift ball
        tx, ty, tee_place_z + hover_z, best_pitch, 2, 0;   % move above tee
        tx, ty, tee_place_z,           best_pitch, 3, 0;   % release on tee
        tx, ty, tee_place_z + hover_z, best_pitch, 0, 0;   % retract
        home_x, home_y, home_z,        0,          0, 0;   % back toward home corridor
    ];
    ...
    waypoints_club = [
        cx, cy, club_pick_z + hover_z, best_pitch_club, 0, 1;   % approach club
        cx, cy, club_pick_z,           best_pitch_club, 4, 1;   % close on club
        cx, cy, club_pick_z + hover_z, best_pitch_club, 5, 1;   % lift club
        home_x, home_y, home_z,        0,               5, 1;   % move away holding club
    ];
...
%% 8. MOVE TO SWING START
fprintf('\n--- Moving to Swing Start ---\n');

% First move to a HIGH HOME to clear the table safely
HOME_HIGH = [0.20, 0.00, 0.25];  home_pitch = 0;
current_pitch_val = sum([current_q(2)+delta, current_q(3), current_q(4)]);
...
% Phase A: follow-through -> home high
NUM_RETURN = 25;
for s = 1:NUM_RETURN
    frac = s / NUM_RETURN;
...
function [current_q, current_gripper, attached_ball, attached_club] = execute_waypoints( ...
    ...
    ball_holders, golf_tee, golf_club_base, unit, rx, ry, ...
    GRIPPER_OPEN, GRIPPER_CLOSE_BALL, GRIPPER_CLOSE_CLUB)
...
% Draw balls (if selected_ball_idx is valid)
if selected_ball_idx > 0
    ...
    % If ball has been hit (idx=-1), just draw the remaining balls based on ball_holders
    for k = 1:size(ball_holders,1)
        [bx, by, ~] = grid_to_world(ball_holders(k,1), ball_holders(k,2), 0, unit, rx, ry);
        if k ~= 1 % Assuming 1 was selected previously, don't draw it
             plot3(bx, by, 0.02, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
        end
    end
end
...
if attached_club
    % show club attached to robot tip
    line([P_tip(1) P_tip(1)], [P_tip(2) P_tip(2)], [P_tip(3)-0.09 P_tip(3)+0.01], ...
        'Color', [0.85 0.65 0.13], 'LineWidth', 4);
else
    % show standing club
    line([cx cx], [cy cy], [0 0.10], 'Color', [0.85 0.65 0.13], 'LineWidth', 4);
end

% Check if ball is placed on tee (if selected and not holding, we placed it)
% In our scenario, after Task A, ball is on tee, attached_ball=false.
% So we draw a ball on the tee. But if idx=-1 (ball hit), we don't draw it.
if selected_ball_idx > 0 && ~attached_ball
    % Are we physically at Task A completed? Yes, we draw ball on tee.
    % To be more robust, we draw it on the tee.
