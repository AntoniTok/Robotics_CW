% task_golf_real.m
% Pick one ball from holder -> place on golf tee -> pick golf club
...
%% 2. GRIPPER SETTINGS
% You may need to tune these for your real ball and golf club
GRIPPER_OPEN       = deg2rad(-45);
GRIPPER_CLOSE_BALL = deg2rad(10);
...
% Ball holders (three possible starting positions)
ball_holders = [
    17, 1;
    17, 2;
    17, 3
...
% Choose which ball to pick: 1, 2, or 3
selected_ball_idx = 1;

% Tee location
golf_tee = [9, 12];

% Golf club base location
golf_club_base = [1, 1];

% Object heights
ball_pick_z   = 0.020;   % adjust if needed for your ball holder height
tee_place_z   = 0.030;   % adjust so ball sits correctly on tee
club_height   = 0.10;    % 10 cm
club_pick_z   = 0.09;    % pick 9 cm from bottom
...
fig = figure('Name','Golf Task: Ball to Tee then Pick Club','Color','w','Position',[100 100 1200 800]);
...
for wp_idx = 1:size(waypoints,1)
    ...
    goal_pitch  = target(4);
    action      = target(5);
    task_type   = target(6); % 0 = ball, 1 = club
